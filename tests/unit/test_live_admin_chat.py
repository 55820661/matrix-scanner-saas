...
class LiveAdminChatTests(TestCase):
    ...
    def test_chatkit_static_asset_is_discoverable_and_uses_custom_server_options(self):
        ...
        self.assertIn("chatkit.fetchUpdates()", source)
        self.assertIn("typeof chatkit.fetchUpdates !== \"function\"", source)
        self.assertIn("ChatKit fetchUpdates() is unavailable for diagnostic bundle sync.", source)
        self.assertIn("ChatKit fetchUpdates() failed for diagnostic bundle sync.", source)
        self.assertIn("const completedBundleIds = new Set()", source)
        self.assertIn("const fetchedBundleIds = new Set()", source)
        ...
        self.assertNotIn("await mountChatKit();\n                if (!updatesSynced)", source)
