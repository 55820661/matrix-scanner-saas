...
## 2026-06-27 - C10.10-H5 Use ChatKit Native Updates for Diagnostic Bundles Start

Intent:
- Make final diagnostic bundle summaries appear through native ChatKit synchronization instead of custom fallback rendering.
- Use `chatkit.fetchUpdates()` as the primary client update path after bundle completion.

Scope:
- Rework diagnostic bundle polling in `live_chatkit.js` to call `fetchUpdates()` on final states.
- Keep fallback rendering only as a last resort when `fetchUpdates()` is unavailable or fails.
- Verify stored final bundle messages remain visible in ChatKit history/store with stable item IDs and no suppression.

Out of scope:
- Migrations, Portal, Telegram, customer-facing AI, write/remediation/shell actions, raw logs, raw JSON, and secrets.

## 2026-06-27 - C10.10-H5 Use ChatKit Native Updates for Diagnostic Bundles Complete

Result:
- Switched diagnostic bundle completion handling to use native `chatkit.fetchUpdates()` as the primary sync path.
- Stopped using remount/reload as the normal completion behavior; fallback rendering remains only when `fetchUpdates()` is unavailable or throws.
- Added guarded warning paths for missing or failed `fetchUpdates()` calls.
- Kept bundle final-message visibility aligned with the existing ChatKit store/history mapping, stable `chatkit_item_id`, and no per-tool bundle result messages.
- No migrations or changes to Portal, Telegram, customer-facing AI, write/remediation behavior, raw logs, raw JSON, or secrets were made.
...
## 2026-06-27 - C10.12-A Admin Chat Workspace Cleanup & Tool Activity Tracking Start

Intent:
- Clean the Admin Internal Chat workspace, move Tool Builder and Reports forms out of the chat page, and make Tool Activity show succeeded, failed, timeout, and skipped tools for the session.

Scope:
- Refactor the admin chat templates and add dedicated pages for Tool Builder and Reports.
- Build a read-only Tool Activity view-model that includes skipped diagnostic bundle tools even when no `ToolRun` exists.
- Normalize Tool Activity statuses and reasons into clean user-facing text without raw JSON, Python lists, or tracebacks.

Out of scope:
- New tools, ToolPolicy or PlanTool semantics, write/remediation behavior, Portal or Telegram AI changes, ChatKit ownership changes, diagnostic bundle lifecycle changes, and migrations.

## 2026-06-27 - C10.12-A Admin Chat Workspace Cleanup & Tool Activity Tracking Complete

Result:
- Cleaned the Admin Internal Chat page so it now focuses on Live Admin AI, selected context, Tool Activity, and links out to Tool Builder and Reports.
- Added a read-only Tool Activity view-model that normalizes succeeded, failed, timeout, running, not-started, and skipped bundle tools without exposing raw JSON, Python lists, or traceback-like payloads.
- Included skipped diagnostic bundle tools in Tool Activity even when they never produced a `ToolRun`, by reading bundle metadata across the session.
- Moved Tool Builder and Reports forms to dedicated admin chat pages while keeping the underlying create endpoints and execution model intact.
- Kept ChatKit ownership, diagnostic bundle lifecycle, tool execution behavior, Portal, Telegram, and migrations unchanged.
